...
| 4 | Hook event `WorktreeCreate` | **supported** | doc / test-verified | Exists. The kit's `worktree_freeze.py` handler + its plugin `hooks.json` wiring are test-verified (`test_lifecycle_hooks.py`, `test_plugin_manifest.py`) under a plugin-style layout with `${CLAUDE_PLUGIN_ROOT}` (ISSUE-023). The **live event firing** still wants one real-session probe before removing the imperative `wt_setup.sh` fallback (do it during ISSUE-026 when the plugin is actually installed). | ISSUE-016 / 023 |
