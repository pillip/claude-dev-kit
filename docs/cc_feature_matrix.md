...
| 2 | Agent `model: inherit` | **supported — kit default since ISSUE-030** | doc | Resolves via normal model resolution instead of overriding. **All 33 core agents ship without `model:` (= inherit)**; a pin may return only with an adjacent `# pin:` rationale (enforced by `test_agent_effort.py`). Deterministic deployments pin once at session/project level (`model` setting / `--model`). | ISSUE-015 / 030 |
...
| 4e | Hook event `SessionStart` (stdout → session context) | **supported** | doc + local (probes 2026-07-22/23) | Fires in headless `-p` sessions too (live-probed via a marker-file hook during ISSUE-035; plugin `hooks.json` + `${CLAUDE_PLUGIN_ROOT}` substitution confirmed). Hook stdout is injected into session context once — the kit uses it for the update check + contributor-mode instructions (`session_start.py`), replacing per-skill preamble steps. | ISSUE-032 |
