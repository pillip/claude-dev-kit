...
5. **ISSUE-026 — Distribution & namespacing (1d)**: ship `.claude-plugin/marketplace.json` (repo = marketplace, listing core + sales); document `/plugin marketplace add` + `/plugin install` UX, the actual `/claude-dev-kit:<skill>` namespace, and the submodule short-name option in README.
...
- [x] Should the kit publish to a private git marketplace or the skills-directory plugin channel for team distribution? — **Resolved (2026-06-22): the repo itself is the marketplace** (`.claude-plugin/marketplace.json`), installed via `/plugin marketplace add pillip/claude-dev-kit`. Namespace is the plugin name (`/claude-dev-kit:<skill>`); the submodule install remains the short-name (`/implement`) path. See ISSUE-026.
