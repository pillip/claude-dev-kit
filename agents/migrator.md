...
1. **Assess scope**: Identify what is being migrated (dependency, DB schema, runtime version, framework, etc.). Check recalled **review lessons** (native memory; passed in your prompt when you run as a subagent) for known recurring migration pitfalls to avoid.
