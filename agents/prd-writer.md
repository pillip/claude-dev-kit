...
1. **Check brainstorm notes**: If `docs/brainstorm_notes.md` exists, read it first. Also check recalled **review lessons** (native memory; passed in your prompt when you run as a subagent) for recurring product issues to address in the PRD. Use the problem space, target users, and chosen direction as starting context. Skip questions already answered there.
