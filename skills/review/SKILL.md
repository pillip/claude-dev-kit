...
3) Gather review context:
   **Read all applicable context files via parallel Read tool calls in a single message.**
   - **Recalled review lessons** — recurring patterns recorded by past reviews auto-recall from native memory into this session; apply them (the legacy `docs/review_lessons.md` is retired, ISSUE-033).
