...
3.7) **Layout verification** (auto-skips if no Figma data):

> **CHECKPOINT — MANDATORY — NEVER SKIP**
> Run: `bash scripts/checkpoint.sh --skill review --phase layout --issue $ARGUMENTS`
> Verifies that element spatial relationships match Figma: container directions (row/column), sibling ordering (A left-of B, C above D), element containment.
> Catches "tokens and structure are correct but everything is in the wrong place."
> If exit code ≠ 0: STOP immediately. Fix layout before proceeding.

3.8) **Visual diff — ADVISORY** (generates diff images for reviewer, does NOT block):
