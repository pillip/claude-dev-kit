...
3.1) **Runtime probe** (per SPEC-019 platform-first delegation):
   ...
   - `CR_EXIT` in {0, 2} → primary path for code dimension (invoke runtime `/code-review`).
   - `CR_EXIT` == 1 → degraded path for code dimension (invoke `reviewer` agent with `--dimension code`).
   - Same for `SR_EXIT` with `/security-review` / `--dimension security`.
...
3.2) **Over-engineering audit (minimality axis)** — kit-distinctive, runs on BOTH paths:
   ...
   - Pass gathered context (review_lessons, architecture) so recurring patterns are recognized.
...
5.5) **Learning Extraction** — runs on the MERGED notes (kit-distinctive, not duplicated by runtime). Identify findings in `docs/review_notes/$ARGUMENTS.md` that could have been prevented earlier (at kickoff or implementation time). Classify each into Code Quality / Security / Testing / Architecture / Over-Engineering. Update `docs/review_lessons.md`:
   - For existing patterns: increment Frequency and append the current issue to Observed-In.
   - For new patterns: create a new entry with the next `[RL-NNN]` ID.
