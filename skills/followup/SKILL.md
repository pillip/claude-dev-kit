...
3) Check inputs:
   ...
   - `docs/sales_email_persona.md` (optional — sales-specific email tone/style; if present, agent will follow it instead of default professional-warm tone)
   - `docs/sales_lessons.md` (optional — cross-account patterns to apply; agent may also draft new lesson candidates from this meeting)

### If followup does NOT exist (New Followup):
4a) Invoke the `champion-mapper` agent via the Task tool, passing meeting notes, account brief, and (if present) email persona + sales lessons paths.
5a) The agent will:
    - Read all inputs + `templates/followup.md` for structure
    - Draft customer-facing email (personalized, references specifics, restates pain in their words). **If `docs/sales_email_persona.md` exists, apply its tone/salutations/sign-off/length rules.** Otherwise default to professional-warm Korean.
